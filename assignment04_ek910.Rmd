...
for(file_name in file_list) {
  path <- paste("data/gdhy_v1/wheat_spring/", file_name, sep="")
  file <- tidync(path)
...
```{r data}
  ...
  data %>% 
    ...
    geom_sf(data = four_states, fill = NA) + 
...
```{r}
  ...
  data %>% 
    ...
    summarize(lat = weighted.mean(lat, w = yield, na.rm = TRUE), 
              lon = weighted.mean(lon, w = yield, na.rm = TRUE)) %>% 
    ggplot() + 
    geom_point(mapping = aes(x = year, y = lat, color = "blue")) +
    geom_smooth(mapping = aes(x = year, y = lat, color = "blue")) +
    geom_point(mapping = aes(x = year, y = lon / -2.38, color = "orange")) +
    geom_smooth(mapping = aes(x = year, y = lon / -2.38, color = "orange")) +
    scale_y_continuous(
      "Lat", 
      sec.axis = sec_axis(~ . * -2.38, name = "Lon")
    ) + 
    labs( 
